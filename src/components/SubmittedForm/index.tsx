...
function SubmittedForm(props: RouteComponentProps<ListURLParams>) {
    ...
    const fetchTableData = async (formId: string) => {
        const tableData = await ipcRenderer.sendSync('submitted-form-definition', formId);
    ...
    const deleteData = (instanceId: string) => {
        setUpdating(true);
        ipcRenderer.sendSync('delete-instance', instanceId, formId);
        fetchTableData(formId);
    };
    ...
    function stableSort<T>(array: T[]) {
        let stabilizedThis;
        if (order === 'asc') {
            stabilizedThis = array.sort(
                (a: any, b: any) => new Date(a.submission_date).getTime() - new Date(b.submission_date).getTime(),
            );
        } else {
            stabilizedThis = array.sort(
                (a: any, b: any) => new Date(b.submission_date).getTime() - new Date(a.submission_date).getTime(),
            );
        }
    ...
    return (
        <div>
            ...
            <Filter
                ...
                setUpdater={setUpdating}
            ></Filter>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <Button
                    style={{ backgroundColor: '#8ac390', borderColor: '#8ac390', margin: 10 }}
                    onClick={() => exportToExcelForSubmittedData(tableData, fieldWithLabels, formAllChoices)}
                >
                                                                ...
                                                                isColumnObj(colObj) ? (
                                                                    ...
                                                                    >
                                                                        {colObj.action_definition.map(
                                                                            (
                                                                                actionObj: ActionDefinition,
                                                                                actionIndex: number,
                                                                            ) => {
                                                                                if (
                                                                                    rowObj['status'] === 0 &&
                                                                                    (actionObj.form_title === 'edit' ||
                                                                                        actionObj.form_title === 'delete')
                                                                                ) {
